...
#from trapy.ttrapy import listen, accept, dial, recv, send, close
...
def handle(conn, file_path, chunk_size):
    ...
    for chunk in chunked_file(file_path, chunk_size):
        x += send(conn, chunk)
        #logger.info(f'Sended {x} bytes of data')
    close(conn)
...
def make_server(address, file_path, chunk_size):
    ...
    while True:
        try:
            conn = accept(server)
            #logger.info(f'Conection #{x} opened at {conn.source_address}')
            x += 1
...
def make_client(address, file_path):
    ...
    while True:
        ...
        #logger.info(f'Elapsed {int(time.time() - tme)} sec')
        #logger.info(f'chunk received. length: {len(chunk), x}-{x // 1000000}%')
